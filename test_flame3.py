...
num_classes = 2
data_dir = "dataset_flame3" 

class StudentModelInference(nn.Module):
    def __init__(self, num_classes=2, modx='densenet'):
        ...
        if modx == 'squeezenet':
            self.modx = models.squeezenet1_1(pretrained=True)  
    ...
    def forward(self, rgb):
        ...
        return stpatch_logits, stimg_logits, st_feat ,patch_ft_proj #stimg_logits #st_feat_proj, #,
...
def test(dataloader, model, device, output_flag=True):
    ...
    with torch.no_grad():
        for img_rgb, img_label, patch_label, imgid in dataloader:
            ...
            stpatch_logits, stimg_logits, _ , _ = model(img_rgb) #stimg_logits
            end_time = time.time()
    ...
    if output_flag:
        def print_metrics(name, y_true, y_pred, y_score, num_classes, log_file, cm_name):
            ...
            plt.ylabel('True')
            plt.tight_layout()
            plt.savefig(f'OUTPUT_flame3/{cm_name}')
            plt.close()
        ...
        os.makedirs("OUTPUT_flame3", exist_ok=True)
        log_path = os.path.join("OUTPUT_flame3", "flame3metrics_log.txt")
...
if __name__ == "__main__":
    ...
    class CustomImageDataset(Dataset):
        ...
        def load_data(self):
            ...
            rgb_file = f'{self.data_dir}/RGB_flame3.txt' #f'RGB_{self.mode}.txt'
    ...
    test_loader = DataLoader(
                    CustomImageDataset(data_dir=data_dir, patch_label_json= f'{data_dir}/flame3patch_labels_test.json'),
                    batch_size=1,
                    shuffle=False,
...
log_path = os.path.join("OUTPUT_flame3", "metrics_log_params.txt")
...
print("results saved to OUTPUT_flame3")
